...
- **[Laravel code style conventions](https://github.com/laravel/pint/blob/main/resources/presets/laravel.php)** — the easiest way to apply the conventions is to run `composer lint`.

- **Static analysis & types** — the code must pass PHPStan at its maximum level with 100% type coverage.

- **Add tests!** — your pull request won’t be accepted if it doesn’t have tests.

- **Run the full check suite** — `composer test` runs the code-style check, static analysis, type coverage, and unit tests. Make sure it passes before opening a pull request.
